...
w0 = bllioc.getInitialWeightsIOC(length(doc.costFunctionVector)-1, 3);

% Set fixed weights for joint jerk
bllioc.fixedWeights = [10^(1.7); 1; 10^(1.7)];

% Add them
w0 = bllioc.matricizeWeightsForIOC(bllioc.addFixedWeights(bllioc.vectorizeWeightsForIOC(w0)));
...
bllioc = bllioc.runIOCsomeFixedWeights(nT, optimopts);
fprintf("Ended IOC.\n");